...
public class Lp_Nw extends base {

        @BeforeMethod(alwaysRun = true)
            public void redirectToLp_Nw() throws InterruptedException {
                driver.get("https://qa-fe.amityonline.com/lp-nw");
                Thread.sleep(2000);
            }

    @Test(priority = 1)
    public void Enquire_Now() throws Exception {
        test = reports.createTest("Enquire Now Open Form (Lp Nw-Middle East)");
        ...
        // try {
        //     WebElement lp_nw_h1 = wait.until(
        //             ExpectedConditions.visibilityOfElementLocated(
        //                     By.xpath("//h1[contains(text(),'Unlock global learning with our innovative online ')]")));
        //     String actual_h1 = lp_nw_h1.getText().trim();
        //     String expected_h1 = "Unlock Global Learning With Our Innovative Online Degree Programs";
        //     softAssert.assertEquals(actual_h1, expected_h1, "H1 text does not match the expected value.");
        //     test.log(Status.PASS, "H1 text visible and correct: " + actual_h1);

        // } catch (TimeoutException e) {
        //     test.log(Status.FAIL, "H1 element was not visible within the wait time.");
        // } catch (AssertionError e) {
        //     test.log(Status.FAIL, "H1 text did not match. Assertion failed: " + e.getMessage());
        // } catch (Exception e) {
        //     test.log(Status.FAIL, "Unexpected error occurred while verifying H1: " + e.getMessage());
        // }

        try {
            WebElement enquire_text = wait.until(
                    ExpectedConditions.visibilityOfElementLocated(
                            By.xpath("//div[@class='styles_formHeading__CMaWB']")));
            String actual_text = enquire_text.getText().trim();
            String expected_heading = "Connect With Our Experts";
            softAssert.assertEquals(actual_text, expected_heading,
                    "Connect with our experts text does not match the expected value.");
            test.log(Status.PASS, "Connect with our experts text visible and correct: " + actual_text);

        } catch (TimeoutException e) {
            test.log(Status.FAIL, "Connect with our experts element was not visible within the wait time.");
        } catch (AssertionError e) {
            test.log(Status.FAIL, "Connect with our experts text did not match. Assertion failed: " + e.getMessage());
        } catch (Exception e) {
            test.log(Status.FAIL,
                    "Unexpected error occurred while verifying Connect with our experts: " + e.getMessage());
        }
        ...
        try {
            ...
            Lp_Nw.validateLeadSummaryData(lead, randomEmail, randomName, randomMobileNumber, expectedURL, expectedProgram, expectedSpecialization, lsqId, cleanVisited, Enquirysource, utm_medium, softAssert);
            Thread.sleep(5000);
    ...
    public void yellow_strip() throws Exception {
        ...
        try {
            ...
            test.info("Random Email: " + randomEmail);
            test.info("Random Mobile Number: " + randomMobileNumber);
            WebElement mobilefirstInput = driver.findElement(By.xpath("/html[1]/body[1]/div[2]/div[1]/div[1]/div[1]/div[1]/div[2]/div[1]/div[1]/form[1]/div[1]/div[1]/div[1]/div[2]/input[1]"));
            wait.until(ExpectedConditions.elementToBeClickable(mobilefirstInput));
            mobilefirstInput.sendKeys(randomMobileNumber);

            WebElement FullNamefirst = driver.findElement(By.xpath("/html[1]/body[1]/div[2]/div[1]/div[1]/div[1]/div[1]/div[2]/div[1]/div[1]/form[1]/div[1]/div[2]/input[1]"));
            wait.until(ExpectedConditions.elementToBeClickable(FullNamefirst));
            FullNamefirst.sendKeys(randomName);

            WebElement emailfirst = driver.findElement(By.xpath("/html[1]/body[1]/div[2]/div[1]/div[1]/div[1]/div[1]/div[2]/div[1]/div[1]/form[1]/div[2]/div[1]/input[1]"));
            wait.until(ExpectedConditions.elementToBeClickable(emailfirst));
            emailfirst.sendKeys(randomEmail);
            lppages.submit_button_one.click();
            Thread.sleep(3000);
            // START HERE COOKIES
            String decodedVisited = null;
            String cleanVisited = null;
            ...
            try {
                ...
                Thread.sleep(10000);
                WebElement degreefirst = driver.findElement(By.xpath(
                        "/html[1]/body[1]/div[2]/div[1]/div[1]/div[1]/div[1]/div[2]/div[1]/div[1]/form[1]/div[2]/div[2]/div[1]/select[1]"));
                Select select = new Select(degreefirst);
                wait.until(ExpectedConditions.elementToBeClickable(degreefirst));
                degreefirst.sendKeys("PG");
                // wait.until(ExpectedConditions.elementToBeClickable(lppages.degreeElement)).sendKeys("PG");
                wait.until(ExpectedConditions.elementToBeClickable(lppages.program_select_Element))
                        .sendKeys("MASTER OF COMPUTER APPLICATIONS WITH SPECIALIZATION IN CYBER SECURITY");
                lppages.submit_button_twoElement.click();
    ...
    @Test(priority = 3)
	public void ApplyNow_BlueRFI_India() throws InterruptedException {
		test=reports.createTest("Apply Now_BlueCart India (Landing Pages)");
		closescholarshippopup();
        closePopupIfPresent();
        String expectedURL = "";
        String expectedProgram = "MASTER OF COMPUTER APPLICATIONS";
		...
		try {
			// Apply Now_BlueCart
			js.executeScript("arguments[0].scrollIntoView();", lppages.applynowBluecart);
			Thread.sleep(2000);
		
			String randomMobileNumber = "230" + random.getRandomMobileNumber();
			String randomName = "TestQA" + " " + random.GetRandomName();
			String randomEmail = "TestQA_" + random.GetRamdonEmailID() + "@gmail.com";
			...
			WebElement mobilefirstInput = driver.findElements(By.xpath("//input[@placeholder='Enter your no.']")).get(1);
			wait.until(ExpectedConditions.elementToBeClickable(mobilefirstInput));
			mobilefirstInput.sendKeys(randomMobileNumber);

			WebElement FullNamefirst = driver.findElements(By.xpath("//input[@placeholder='Enter your full name']")).get(1);
			wait.until(ExpectedConditions.elementToBeClickable(FullNamefirst));
			FullNamefirst.sendKeys(randomName);

			WebElement emailfirst = driver.findElements(By.xpath("//input[@placeholder='abc@xyz.com']")).get(1);
			wait.until(ExpectedConditions.elementToBeClickable(emailfirst));
			emailfirst.sendKeys(randomEmail);
			lppages.submitbuttonElement.click();
			Thread.sleep(3000);
            // START HERE COOKIES
            String decodedVisited = null;
            String cleanVisited = null;
            Cookie visitedURLsCookie = waitForCookie("VisitedURLs", 5, 3000); // 5 retries, 1s gap
			...
			try {
				...
				WebElement degreefirst = driver.findElement(By.xpath("//div[@class='SelectDark_SelectContainer__TyXTH w-full']//select[@id='degree_lookup']"));
				wait.until(ExpectedConditions.elementToBeClickable(degreefirst));
				degreefirst.sendKeys("PG");
				//wait.until(ExpectedConditions.elementToBeClickable(lppages.degreeElement)).sendKeys("PG");
				wait.until(ExpectedConditions.elementToBeClickable(lppages.prograElement)).sendKeys("MASTER OF COMPUTER APPLICATIONS WITH SPECIALIZATION IN CYBER SECURITY");
				lppages.submitbuttonElement.click();
				test.info("Degree and Program selection completed successfully.");
			} catch (Exception e) {
				test.warning("Degree/Program selection step skipped: " + e.getMessage());
			...
			try {
				...
				WebElement thankYouMessage = wait.until(ExpectedConditions.presenceOfElementLocated(
						By.xpath("//div[@class=' ThankYouBanner_Title__LtMZO']")));
                        //div[@class=' ThankYouBanner_Title__LtMZO']
			...
			// Check for Start Application button
			try {
				WebElement isSuccess = wait.until(ExpectedConditions.elementToBeClickable(lppages.startapplicationElement));
				if (isSuccess.isEnabled()) {
					softAssert.assertTrue(true, "Start Application button enabled check");
					test.log(Status.PASS, "Apply Now_BlueCart Indian Journey Successful");
					System.out.println("Apply Now_BlueCart Indian Journey Successful");
				} else {
					test.log(Status.FAIL, "Start Application button not enabled");
					softAssert.fail("Start Application button not enabled");
					System.out.println("Apply Now_BlueCart Indian Journey Failed");
				}
			} catch (TimeoutException e) {
				test.log(Status.FAIL, "Start Application button did not appear: " + e.getMessage());
				softAssert.fail("Start Application button not found");
				System.out.println("Apply Now_BlueCart Indian Journey Failed - Button not found");
			}
            String student_login = driver.getCurrentUrl();
            // Remove http:// or https:// from both URLs
            String actualTrimmed = student_login.replaceFirst("^https?://", "");
			...
			test.log(Status.FAIL, "Apply Now_BlueCart Indian Flow Failed: " + e.getMessage());
			softAssert.fail("Apply Now_BlueCart Indian Flow Failed - " + e.getMessage());
		} finally {
            // === ENSURE COOKIES ARE CLEARED EVEN IF TEST FAILS ===
            driver.manage().deleteAllCookies();
            test.info("🧹 All cookies cleared in finally block.");
    ...
    @Test(priority = 4)
	public void ApplyNow_BlueRFI_International() throws InterruptedException {
		test=reports.createTest("Apply Now_BlueCart International (Landing Pages)");
		closescholarshippopup();
        closePopupIfPresent();
        String expectedURL = "";
        String expectedProgram = "MASTER OF COMPUTER APPLICATIONS";
		...
		try {
            ...
            WebElement unitedElement = wait.until(ExpectedConditions.elementToBeClickable(
                        By.xpath("//div[@class='CustomDropdownGlobal_countryCode__yswjf CustomDropdownGlobal_darkText__HHfKn']")));
            ...
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true); arguments[0].click();", unitedElement);
            WebElement uae = driver.findElement(By.xpath("//div[@class='CustomDropdownGlobal_dropdownMenu__cyPE8']//div[2]//span[1]"));
            uae.click();
